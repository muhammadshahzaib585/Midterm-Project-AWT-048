    ...
    <div className="min-h-screen bg-[#022c22] text-white flex">
      ...
      <aside className="w-64 border-r border-emerald-900/50 bg-[#011d17] p-6 hidden md:flex flex-col">
        <div className="mb-8 flex items-center gap-2">
          <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-rose-400 to-pink-600 flex items-center justify-center text-xs font-black">AF</div>
          <span className="font-black text-white">AdFlow <span className="text-rose-400">ADMIN</span></span>
        </div>
        <div className="mb-4 px-3 py-1.5 bg-rose-500/10 border border-rose-500/20 rounded-full text-xs font-black text-rose-400 text-center">ADMIN PANEL</div>
        <nav className="space-y-1 flex-1">
          {[
            { href: '/admin', label: 'Overview', icon: '🏠', active: true },
            { href: '/admin/users', label: 'Manage Users', icon: '👥', active: false },
            { href: '/admin/ads', label: 'Manage Ads', icon: '📢', active: false },
            { href: '/admin/payments', label: 'Payments', icon: '💳', active: false },
          ].map(item => (
            <Link key={item.href} href={item.href}
              className={`flex items-center gap-3 px-4 py-3 rounded-xl font-semibold text-sm transition ${item.active ? 'bg-rose-500/10 text-rose-400 border border-rose-500/20' : 'text-emerald-100/50 hover:bg-emerald-900/30 hover:text-white'}`}>
