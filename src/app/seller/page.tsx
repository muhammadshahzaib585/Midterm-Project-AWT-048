    ...
    <div className="min-h-screen bg-[#022c22] text-white flex">
      ...
      <aside className="w-64 border-r border-emerald-900/50 bg-[#011d17] p-6 hidden md:flex flex-col">
        <div className="mb-8 flex items-center gap-2">
          <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-amber-400 to-orange-600 flex items-center justify-center text-xs font-black">AF</div>
          <span className="font-black text-white">AdFlow <span className="text-amber-400">SELLER</span></span>
        </div>
        <div className="mb-4 px-3 py-1.5 bg-amber-500/10 border border-amber-500/20 rounded-full text-xs font-black text-amber-400 text-center">SELLER PORTAL</div>
        <nav className="space-y-1 flex-1">
          {[
            { href: '/seller', label: 'Dashboard', icon: '🏠', active: true },
            { href: '/client/ads/create', label: 'Post New Ad', icon: '➕', active: false },
            { href: '/seller/earnings', label: 'My Earnings', icon: '💰', active: false },
            { href: '/seller/analytics', label: 'Analytics', icon: '📈', active: false },
            { href: '/packages', label: 'Packages', icon: '📦', active: false },
          ].map(item => (
            <Link key={item.href} href={item.href}
              className={`flex items-center gap-3 px-4 py-3 rounded-xl font-semibold text-sm transition ${item.active ? 'bg-amber-500/10 text-amber-400 border border-amber-500/20' : 'text-emerald-100/50 hover:bg-emerald-900/30 hover:text-white'}`}>
